...
class PullAAAData(object):
    class Meta:
        ...
        states = ['vic',]
        save_location = os.path.join(os.getcwd(),'data/aaa_site')
    ...
    def __init__(self, *args, **kw):
        ...
        if not os.path.exists(self.Meta.save_location):
            print "save file doesnt exist, making now"
            os.makedirs(self.Meta.save_location)



    def get_site_html(self):
        request = urllib2.Request(self.Meta.url)
    ...
    def download(self):
        ...
        for key, items in self.download_links.iteritems():
            print "downloading %s file [%s] from %s" % (
                                                    ...
                                                    items['file_name'],
                                                    items['link'])
            save_as = os.path.join(self.Meta.save_location, items['file_name'])
    ...
    def xl_to_json(self):
        ...
        import json
        current_files = os.listdir(self.Meta.save_location)
        print current_files
        for xl_file in current_files:
            xl_file_directory = os.path.join(self.Meta.save_location, xl_file)
